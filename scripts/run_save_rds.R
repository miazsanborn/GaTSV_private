...
if (grepl("\\.(txt|tsv|csv)$", opt$metadata, ignore.case = TRUE)) {
...
#filt_file <- filter("/mnt/storage/dept/medonc/beroukhim/mia/GaTSV/data/example.sv.vcf", "example")
#print(nrow(filt_file))  # if this is 0, everything got filtered out
save_rds(file_path,name,n_cores=cores,genome=gnm,output_path = out)
